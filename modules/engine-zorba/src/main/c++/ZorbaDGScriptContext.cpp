...
#include <io_dgms_unity_modules_engines_zorba_ZorbaDGScriptContext.h>
#include <ZorbaDGScriptContext.h>
...
JNIEXPORT jboolean JNICALL Java_io_dgms_unity_modules_engines_zorba_ZorbaDGScriptContext_construct(
	...
	try {
		...
		if (sctx == NULL)
			return JNI_FALSE;
		ZorbaDGResolver* resolver = new ZorbaDGResolver(env, obj);
		jclass cls = env->GetObjectClass(obj);
...
JNIEXPORT jboolean JNICALL Java_io_dgms_unity_modules_engines_zorba_ZorbaDGScriptContext_destroy(
	...
	try {
		...
		if (resolverCPtr != 0)
			delete (ZorbaDGResolver*) resolverCPtr;
		return JNI_TRUE;
	} catch (...) {
		return JNI_FALSE;
	}
...
ZorbaDGExternalFunction::ZorbaDGExternalFunction(JavaVM* javaVM, jobject obj,
		ZorbaDGExternalModule* module, const std::string name) :
		javaVM(javaVM), obj(obj), module(module), name(name) {
}

ZorbaDGExternalFunction::~ZorbaDGExternalFunction() {
}

zorba::ItemSequence_t ZorbaDGExternalFunction::evaluate(
...
zorba::String ZorbaDGExternalFunction::getLocalName() const {
	return name;
}

zorba::String ZorbaDGExternalFunction::getURI() const {
	return module->getURI();
}

ZorbaDGExternalModule::ZorbaDGExternalModule(JNIEnv * env, jobject obj,
...
ZorbaDGExternalModule::~ZorbaDGExternalModule() {
	JNIEnv* env = NULL;
	try {
		if (obj == NULL)
			return;
	...
	javaVM->DetachCurrentThread();
	std::map<std::string, ZorbaDGExternalFunction*>::iterator i;
	for (i = functions.begin(); i != functions.end(); i++)
		if (i->second != NULL)
			delete i->second;
}

zorba::ExternalFunction* ZorbaDGExternalModule::getExternalFunction(
		const zorba::String& name) {
	ZorbaDGExternalFunction* function = functions[name.str()];
	if (function != NULL)
		return function;
	function = new ZorbaDGExternalFunction(javaVM, obj, this, name.str());
	functions[name.str()] = function;
	return function;
}

zorba::String ZorbaDGExternalModule::getURI() const {
	return identifier;
}

ZorbaDGResolver::ZorbaDGResolver(JNIEnv * env, jobject obj) {
	env->GetJavaVM(&javaVM);
	this->obj = env->NewGlobalRef(obj);
}

ZorbaDGResolver::~ZorbaDGResolver() {
	JNIEnv* env = NULL;
	try {
		if (obj == NULL)
			return;
...
zorba::URIMapper::Kind ZorbaDGResolver::mapperKind() {
	return (zorba::URIMapper::Kind) 1;
}

zorba::Resource* ZorbaDGResolver::resolveURL(const zorba::String& aUrl,
...
void ZorbaDGResolver::mapURI(const zorba::String aUri,
		zorba::EntityData const* aEntityData,
...
JNIEXPORT jboolean JNICALL Java_io_dgms_unity_modules_engines_zorba_ZorbaDGScriptContext_register(
	...
	try {
		...
		ZorbaDGExternalModule* module = new ZorbaDGExternalModule(env, obj,
				nsUTFChars);
