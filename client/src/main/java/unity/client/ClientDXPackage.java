 ...
 *
 */
public class ClientDXPackage extends ClientDXSessionObject implements DXPackage
{
    /**
     *
     */
     ...
     */
    public ClientDXPackage(ClientDXSession session, ClientDXRegistry registry, JsonObject object,
    ...
    private void resolveDependencies(JsonObject object)
    {
        object.getAsJsonObject("dependencies").entrySet().stream()
                .map(d -> new ClientDXPackageReference(d.getKey(), d.getValue().getAsString())).forEach(d -> {
                    if (dependencies.containsKey(d.getName()))
                        return;
                    try {
                        final ClientDXPackage dependency = registry.getPackage(d, false);
