 ...
 *
 */
public class ClientDGPackage extends ClientDGSessionObject implements DGPackage
{
    /**
     *
     */
     ...
     */
    public ClientDGPackage(ClientDGSession session, ClientDGRegistry registry, JsonObject object,
    ...
    private void resolveDependencies(JsonObject object)
    {
        object.getAsJsonObject("dependencies").entrySet().stream()
                .map(d -> new ClientDGPackageReference(d.getKey(), d.getValue().getAsString())).forEach(d -> {
                    if (dependencies.containsKey(d.getName()))
                        return;
                    try {
                        final ClientDGPackage dependency = registry.getPackage(d, false);
