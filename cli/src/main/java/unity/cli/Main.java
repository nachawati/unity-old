 ...
 *
 */
@Parameters(commandDescription = "Unity")
public final class Main
{
     ...
     */
    public static TransportConfigCallback getTransportConfigCallback(DXSession session)
    {
        return transport -> {
     ...
     * @return KeyPair
     */
    public static KeyPair loadKeyPair(DXSession session, URI gitLabHost, JSch jsch)
    {
    ...
    public static void main(String... args)
    {
        ...
        try {
            ...
            jCommander.setProgramName("unity");
            ...
            if (main.help != null && main.help)
                ...
                System.out.println("Unity, Version " + version);
            } else if (commands
     ...
     */
    public static SshSessionFactory newSshSessionFactory(DXSession session)
    {
     ...
     * @throws IOException
     */
    public static DXSession restore() throws IOException
    {
        final Gson gson = new Gson();
        try (Reader reader = Files.newBufferedReader(getSystemPath().resolve("session.json"))) {
            ...
            if (sessionProperties.has("gitLabHost")) {
                ...
                return new UnityDXSession(gitLabHost, privateToken);
            } else {
                final URI host = URI.create(sessionProperties.get("host").getAsString());
                return new ClientDXSession(host, privateToken);
            }
        } catch (final Exception e) {
            return null;
        }
     ...
     * @throws IOException
     */
    public static void saveKeyPair(DXSession session, URI gitLabHost, KeyPair keyPair) throws DXException, IOException
    {
     ...
     * @throws IOException
     */
    public static DXCommit synchronize(DXSession session, Path path) throws DXException
    {
        try {
            ...
            try (final Git git = Git.wrap(new RepositoryBuilder().findGitDir(rootPath.toFile()).build())) {
                ...
                if (!repositoryPath.startsWith(Main.getSystemPath().resolve("git")))
                    throw new DXException(ansi().fgBright(YELLOW).a(path.getFileName().toString()).reset()
                            + " is not managed by Unity");
                final String projectPath = repositoryPath.toString()
                        .substring(Main.getSystemPath().resolve("git").toString().length() + 1).replace("\\", "/");
                final DXProject project = session.getSystem().getProject(projectPath);
