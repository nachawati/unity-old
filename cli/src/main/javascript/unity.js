...
var jarName = "unity-cli-${project.version}.jar";
...
classPath.forEach(function(part) {
    ...
    first = false;
    classPathStr += __dirname + path.sep + part;
});
...
if (jarName !== '{'+'{JAR_NAME}}') {
	args = args.concat(['-jar', __dirname+path.sep+".."+path.sep+"lib"+path.sep+jarName]);
    //cmd += ' -jar "'+__dirname+'/'+jarName+'" ';
} else {
	args = args.concat(['-cp', classPath, mainClass]);
...
var child = spawn(cmd, args, {stdio: 'inherit', encoding: 'utf8'});
